...
class Player {
  ...
  update(keysLeft, keysRight, keysJump) {
    if (this.reversal) {
      ...
      if (this.history.length > 360) this.history.shift(); // 6s at 60fps
    }
...
function tagCheck() {
  ...
  if (Math.abs(dx) < 30 && Math.abs(dy) < 30) {
    if (!player1.reversal && !player2.reversal) {
      if (player1.tagger && !player2.shield) {
        player1.tagger = false;
        player2.tagger = true;
      } else if (player2.tagger && !player1.shield) {
